...
cd.campaign_topic, 
cd.platform
FROM `x-marketing.terrasmart_googlesheet.Campaign_Topic_Grouping` cd
), campaign AS (
...
CROSS JOIN campaigns
  ...
  SELECT 
      ...
      FROM (  
        SELECT
          DISTINCT
           COALESCE(_campaignID,'') AS _campaignID,
          _domain AS _domain,
          #email
...
--             IF(_form_filled_score > 42, 42,_form_filled_score) ) 
 ...
 ,final_scoring AS (
   ...
   platform,
   campaign._domain,
   _ebos, _utilityprojects, _tracker, _linkedinurl, _canopy, _prospect, _fixedtilt, _account, _persona, _rep, _midwest ,_account_segment,_type,
...
COALESCE(_distinct_experience_event_attendance, 0 ) AS _distinct_experience_event_attendance,
 ...
 FROM campaign
 LEFT JOIN 
 contact_score_limit ON campaign._campaignID = contact_score_limit._campaignID AND campaign._domain = contact_score_limit._domain
 ) SELECT  *,
