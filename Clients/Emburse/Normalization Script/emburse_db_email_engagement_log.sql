...
---Temporary table as the old one still needed atm. Soon will replace this to the db_email_engagements_log once all done between old and new marketo
TRUNCATE TABLE `x-marketing.emburse.temp_db_email_engagements_log`;
INSERT INTO `x-marketing.emburse.temp_db_email_engagements_log` (
  _sdc_sequence,
  _campaignID,
  _campaign,
  _subject,
...
prospect_info_base AS (
  SELECT
    *
  FROM prospect_info_old
  UNION ALL
  SELECT 
    * 
  FROM prospect_info_new
),
---Remove duplicate leads that may appear from both old and new marketo (priority on the details in new marketo)
prospect_info AS (
  SELECT
    *
  FROM prospect_info_base
  QUALIFY ROW_NUMBER() OVER(
    PARTITION BY _email 
    ORDER BY _id DESC) = 1
),
...
email_delivered AS (
  SELECT
    *
  FROM email_delivered_old
  UNION ALL
  SELECT
    *
  FROM email_delivered_new
),
email_open_old AS (
  SELECT
    _sdc_sequence,
...
email_click AS (
  SELECT
    *
  FROM email_click_old
  UNION ALL
  SELECT
    *
  FROM email_click_new
),
open_click AS ( --merge open and click data
  SELECT 
    * 
...
email_hard_bounce AS (
  SELECT
    *
  FROM email_hard_bounce_old
  UNION ALL
  SELECT
    *
  FROM email_hard_bounce_new
),
email_soft_bounce_old AS (
  SELECT
    _sdc_sequence,
    CAST(primary_attribute_value_id AS STRING) AS _campaignID,
    primary_attribute_value AS _campaign,
...
email_soft_bounce AS (
  SELECT
    *
  FROM email_soft_bounce_old
  UNION ALL
  SELECT
    *
  FROM email_soft_bounce_new
),
email_soft_hard_bounced AS (
  SELECT 
    * 
...
email_download AS (
  SELECT
    *
  FROM email_download_old
  UNION ALL
  SELECT
    *
  FROM email_download_new
),
email_unsubscribed_old AS (
  SELECT
    _sdc_sequence,
    CAST(primary_attribute_value_id AS STRING) AS _campaignID,
    primary_attribute_value AS _campaign,
...
email_unsubscribed AS (
  SELECT
    *
  FROM email_unsubscribed_old
  UNION ALL
  SELECT 
    *
  FROM email_unsubscribed_new
),
engagements_combined AS (
  SELECT 
    * 
