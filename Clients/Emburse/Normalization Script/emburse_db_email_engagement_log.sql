...
email_open AS (
  ...
  --WHERE primary_attribute_value LIKE '2024-NN-EMB-APAC-MM-ENT-Short-Term-Disposition-Cold-Opp.Email #1'
  QUALIFY ROW_NUMBER() OVER(PARTITION BY leadid, primary_attribute_value_id ORDER BY activitydate DESC) = 1
),
email_click AS (
  SELECT
...
--SELECT * FROM new_open_consolidate
--WHERE _campaign = '2024-NN-EMB-APAC-MM-ENT-Short-Term-Disposition-Cold-Opp.Email #1',
