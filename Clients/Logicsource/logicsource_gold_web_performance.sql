...
WITH mouseflow_recording AS (
  SELECT DISTINCT
    ...
    ROW_NUMBER() OVER (
      PARTITION BY _domain, _recordingid, _visitorid, EXTRACT(DATE FROM _timestamp)
      ORDER BY _timestamp
    ) AS _order
...
webtrack_6sense AS (
  SELECT
    ...
    CASE
      WHEN _6sense._category IS NULL
      AND _6sense._category2 IS NULL THEN ''
      WHEN _6sense._category = ''
      AND _6sense._category2 = '' THEN ''
...
LEFT JOIN mouseflow_pageviews 
  ...
  AND mouseflow_recording._key = mouseflow_pageviews._key
...
UPDATE `x-marketing.logicsource.dashboard_mouseflow_kickfire` logicsource_mflow
SET
