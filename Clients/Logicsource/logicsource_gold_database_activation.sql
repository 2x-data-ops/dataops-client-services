...
WITH dummy_dates AS (
  ...
  FROM UNNEST (GENERATE_DATE_ARRAY('2019-12-01', CURRENT_DATE(), INTERVAL 1 DAY)) AS _date
),
tam_database AS (
  SELECT DISTINCT
...
companies AS (
  SELECT
    MIN(property_createdate.value) AS _createddate,
...
LEFT JOIN account_engagement
  ON dummy_dates._date = account_engagement._date
LEFT JOIN accounts_created
  ON dummy_dates._date = DATE(accounts_created._createddate)
LEFT JOIN contacts_created
  ON dummy_dates._date = DATE(contacts_created._createddate)
ORDER BY _date DESC;