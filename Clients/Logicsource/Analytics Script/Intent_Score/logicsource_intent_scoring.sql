-- CREATE OR REPLACE TABLE `x-marketing.logicsource.zoominfo_intent_score` AS
...
dates AS (
  SELECT DISTINCT
    CAST(_lastsignal AS DATE) AS _date,
    CAST(_exporteddate AS DATE) AS _exporteddate,
...
all_account AS (
  SELECT
    acc.*,
    report._score AS _intent_score,
    report._topic,
    _date,
    _exporteddate,
  ...
  FROM account AS acc --CROSS JOIN dummy_date 
  LEFT JOIN zoominfo_intent AS report
    ON acc._domain = report._domain
...
selected_engagements_total AS (
  SELECT
    ...
    SUM(
      CASE
        WHEN _engagement = 'Email Opened' THEN 1
        ELSE 0
      END
    ) AS _emailOpened,
    SUM(
      CASE
        WHEN _engagement = 'Email Clicked' THEN 1
        ELSE 0
      END
    ) AS _emailClicked,
    SUM(
      CASE
        WHEN _engagement = 'Form Filled' THEN 1
        ELSE 0
      END
    ) AS _formfilled,
    SUM(
      CASE
        WHEN _engagement = 'Paid Ads' THEN 1
        ELSE 0
      END
    ) AS _paidads,
    SUM(
      CASE
        WHEN _engagement = 'Organic Social' THEN 1
        ELSE 0
      END
    ) AS _organicsocial,
    SUM(
      CASE
        WHEN _engagement = 'Web Visit' THEN 1
        ELSE 0
      END
    ) AS _webvisit,
  ...
  FROM selected_engagements --WHERE _domain = 'fedex.com'
  GROUP BY 1, 2, 3
),
_engagement AS (
  SELECT
  ...
  FROM selected_engagements_total --WHERE _domain = 'foodtravelexperts.com'
)
SELECT
  all_account.*,
  _engagement.* EXCEPT (_domain, _week, _year),
  CASE
    WHEN _source = 'Target' THEN "Existing"
    ELSE "Net New"
  END AS _account_status,
  CAST(NULL AS STRING) AS _intent
FROM
  all_account
LEFT JOIN _engagement
  ON (
  all_account._domain = _engagement._domain
  AND all_account._week = _engagement._week
  AND all_account._year = _engagement._year
);

#Set NULLS to 0 for aggregation in
UPDATE `x-marketing.logicsource.zoominfo_intent_score`
SET
  running_opened = CASE
    WHEN running_opened IS NOT NULL THEN running_opened
    ELSE 0
  END,
  running_clicked = CASE
    WHEN running_clicked IS NOT NULL THEN running_clicked
    ELSE 0
  END,
  running_formfilled = CASE
    WHEN running_formfilled IS NOT NULL THEN running_formfilled
    ELSE 0
  END,
  running_paidads = CASE
    WHEN running_paidads IS NOT NULL THEN running_paidads
    ELSE 0
  END,
  running_organicsocial = CASE
    WHEN running_organicsocial IS NOT NULL THEN running_organicsocial
    ELSE 0
  END,
  running_webvisit = CASE
    WHEN running_webvisit IS NOT NULL THEN running_webvisit
    ELSE 0
  END,
  _avgCompositeScore = CASE
    WHEN _avgCompositeScore IS NOT NULL THEN _avgCompositeScore
    ELSE 0
  END,
  max_score = CASE
    WHEN max_score IS NOT NULL THEN max_score
    ELSE 0
  END
WHERE _domain IS NOT NULL;
...
SET
  _intent = CASE
    WHEN
    /* REGEXP_CONTAINS(CAST(_tier AS STRING),'1|2') AND */
    _avgCompositeScore >= 60
    AND _total_score_icp_intent >= 60 THEN "High"
    WHEN
    /* REGEXP_CONTAINS(CAST(_tier AS STRING),'1|2') AND */
    _avgCompositeScore < 60
    AND _total_score_icp_intent >= 60 THEN "High"
    ...
    /* REGEXP_CONTAINS(CAST(_tier AS STRING),'1|2') AND */
    _avgCompositeScore >= 60
    AND _total_score_icp_intent < 60 THEN "Medium"
    WHEN
    /* REGEXP_CONTAINS(CAST(_tier AS STRING),'1|2') AND */
    _avgCompositeScore < 60
    AND _total_score_icp_intent < 60 THEN "Low"
