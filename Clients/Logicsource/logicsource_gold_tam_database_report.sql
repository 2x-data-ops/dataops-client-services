...
INSERT INTO `x-marketing.logicsource.db_tam_database_report` (
  ...
  WITH contacts AS (
    ...
    LEFT JOIN `x-marketing.logicsource_hubspot.contacts` k
      ON lead.id = k.properties.salesforceleadid.value
    LEFT JOIN `x-marketing.logicsource_salesforce.Account` acc
      ON lead.convertedaccountid = acc.id
  ...
  accounts AS (
    ...
    FROM `x-marketing.logicsource_salesforce.Account` acc
    LEFT JOIN companies AS hub
      ON acc.id = hub.salesforceaccountid 
    ...
    LEFT JOIN contacts
      ON acc.id = contacts._sfdcaccountid
  ...
  dummy_dates AS (
    ...
    FROM UNNEST (GENERATE_DATE_ARRAY(CURRENT_DATE(), '2019-12-01', INTERVAL -1 WEEK)) AS _date
  ),
...
SELECT DISTINCT
  ...
  CASE
    ...
    ELSE 'No'
  END _t90days_first_party_breakdown,
  (
    /* CASE
    ...
    END */
    CAST(NULL AS STRING)
  ) AS _third_party_breakdown
FROM account_dates main
LEFT JOIN engagement
  USING (_domain, _week, _year)
LEFT JOIN first_party_score
  USING (_domain, _week, _year)
LEFT JOIN new_weekly_contacts
  USING (_domain, _week, _year)
ORDER BY main._domain ASC, main._year DESC, main._week DESC;
