-- CREATE OR REPLACE TABLE `x-marketing.logicsource.contact_engagement_scoring` AS
...
email_last_engagementdate AS (
  ...
  RIGHT JOIN email_engagements
    ON email_engagements._id = _last_engagement._id
),
form_filled AS (
  SELECT
    _domain,
...
formfill_last_engagementdate AS (
  ...
  RIGHT JOIN formfilled_engagements
    ON formfilled_engagements._id = _last_engagement._id
),
paid_social AS (
  SELECT
    _domain,
    ...
    SUM(
      CASE
        WHEN _engagement = 'Paid Ads'
        AND REGEXP_CONTAINS(LOWER(_contentTitle), 'click|like') THEN 1
        ELSE 0
      END
    ...
    -- EXTRACT(WEEK FROM _timestamp) AS _week, EXTRACT(YEAR FROM _timestamp) AS _year
  FROM `x-marketing.logicsource.db_consolidated_engagements_log`
  WHERE _engagement IN ('Paid Ads') --WHERE _domain = 'fedex.com'
  GROUP BY 1, 2, 3
),
...
paid_social_last_engagement AS (
  ...
  RIGHT JOIN paid_social_engagements
    ON paid_social_engagements._id = _last_engagement._id
),
organic_social AS (
  SELECT
    _domain,
...
organic_social_last_engagement AS (
  ...
  RIGHT JOIN organic_social_engagements
    ON organic_social_engagements._id = _last_engagement._id
),
all_last_engagements AS (
  SELECT
    main.*,
    email_engagement.* EXCEPT (_domain, _email, _id),
  ...
  SELECT
    *,
    (
      ...
      COALESCE(_organic_ads_score_total, 0)
    ) AS _total_score,
    CASE
      WHEN (
...
SELECT
  ...
  CASE
    WHEN COALESCE(_total_score, 0) + COALESCE(total_score_ICP, 0) >= 10
    AND COALESCE(_total_score, 0) + COALESCE(total_score_ICP, 0) <= 20 THEN 'Low'
    WHEN COALESCE(_total_score, 0) + COALESCE(total_score_ICP, 0) >= 21
    AND COALESCE(_total_score, 0) + COALESCE(total_score_ICP, 0) <= 49 THEN 'Medium'
    WHEN COALESCE(_total_score, 0) + COALESCE(total_score_ICP, 0) >= 50 THEN 'High'
    ELSE "Low"
  END AS legend
FROM all_data
LEFT JOIN icp_score
  ON all_data._id = icp_score._id;