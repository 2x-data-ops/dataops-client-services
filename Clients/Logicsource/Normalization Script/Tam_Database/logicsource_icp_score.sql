...
INSERT INTO `x-marketing.logicsource.contact_icp_score` (
 ...
 SELECT
     CAST(vid AS STRING) AS _prospectid,
      ...
      CASE WHEN associated_company.properties.domain.value IS NULL THEN property_hs_email_domain.value ELSE associated_company.properties.domain.value END AS _domain,
      properties.jobtitle.value AS jobtitle,
      properties.job_function.value AS _function,
        CASE WHEN associated_company.properties.name.value IS NULL THEN properties.company.value ELSE  associated_company.properties.name.value END AS _company,
        CASE WHEN associated_company.company_id IS NULL THEN CAST(properties.associatedcompanyid.value AS INT64) END AS company_id,
       ...
       CASE WHEN  property_job_role__organic_.value = '' THEN 0 
       ...
       WHEN property_job_role__organic_.value LIKE '%Business Development%' THEN 0 
          ...
          ELSE 0 END AS score_job_role_organic,
     ...
     CASE WHEN  property_management_level__organic_.value = '' THEN 0 
...
ELSE 10 END anualrevenue_range_score,
 ...
 SUM(score_job_role_organic)+ SUM(score_management_level_organic)+ SUM(anualrevenue_range_score)+  SUM(score_job_role)+ SUM(score_management_level) AS total_score_ICP
 FROM contact
