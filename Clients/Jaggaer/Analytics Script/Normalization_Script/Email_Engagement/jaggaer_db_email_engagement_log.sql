...
LEFT JOIN prospect_info 
  ON CONCAT (engagements._prospectID, LOWER(engagements._email)) = CONCAT (prospect_info._prospectID, LOWER(prospect_info._email))
;
...
UPDATE `x-marketing.jaggaer.db_email_engagements_log` origin  
...
FROM (
  WITH opened_emails AS (
    SELECT
      ...
      _list_email_id,
      _email_id
    FROM `x-marketing.jaggaer.db_email_engagements_log`
  ...
  SELECT DISTINCT
    ...
    ROW_NUMBER() OVER(
    PARTITION BY click._prospectID, click._list_email_id 
    ORDER BY click._timestamp  DESC 
    ) AS rownum  
...
WHERE origin._prospectID = scenario._prospectID
...
QUALIFY ROW_NUMBER() OVER( 
  ...
  ORDER BY prospect.updated_at DESC) = 1
;
