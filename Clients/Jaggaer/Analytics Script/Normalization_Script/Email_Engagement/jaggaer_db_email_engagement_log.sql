...
WITH prospect_info AS (
  SELECT 
    ...
    CASE   
      WHEN LOWER(job_title) LIKE LOWER("%Senior Partner Marketing Manager%") THEN "Manager"
  ...
  FROM `x-marketing.jaggaer_pardot_v4.prospects` prospect
  LEFT JOIN `x-marketing.jaggaer_pardot_v4.campaigns` campaign
    ON prospect.campaign_id = campaign.id
  WHERE email NOT LIKE '%@2x.marketing%' 
    AND email NOT LIKE '%2X%' 
    AND email NOT LIKE '%@jaggaer.com'
  QUALIFY ROW_NUMBER() OVER( 
    PARTITION BY prospect.id,prospect.email 
...
delivered_email AS (
  ...
  FROM `x-marketing.jaggaer_pardot_v4.visitor_activities` activity
  LEFT JOIN `x-marketing.jaggaer_pardot_v4.prospects` prospect 
    ...
    FROM
      (
        ...
        FROM
          ...
          LEFT JOIN `x-marketing.jaggaer_pardot_v3.prospects` prospect ON activity.prospect_id = prospect.id -- JOIN 
          --     airtable_info AS airtable
          -- ON 
          --     CAST(activity.list_email_id AS STRING) = airtable._pardotid
          LEFT JOIN `x-marketing.jaggaer_pardot_v3.campaigns` campaign ON activity.campaign_id = campaign.id
        WHERE
        ...
        FROM
          `x-marketing.jaggaer_pardot_v3.visitor_activities` activity
          LEFT JOIN `x-marketing.jaggaer_pardot_v3.prospects` prospect ON activity.prospect_id = prospect.id
          LEFT JOIN `x-marketing.jaggaer_pardot_v3.campaigns` campaign ON activity.campaign_id = campaign.id -- LEFT JOIN 
          --     airtable_info AS airtable
          -- ON 
          --     CAST(activity.list_email_id AS STRING) = airtable._pardotid
        WHERE
    ...
    FROM
      (
        SELECT
          *
        FROM
          clicks_downloads_timeline
        WHERE
          _engagement = 'Downloaded'
      ) download
      JOIN (
        SELECT
          *
        ...
        WHERE
          _engagement = 'Clicked'
      ) click ON download._prospectID = click._prospectID
      AND EXTRACT(
        DAY
        FROM
          download._timestamp
    ...
    FROM
      (
        ...
        FROM
          `x-marketing.jaggaer_pardot_v3.visitor_activities` activity
          LEFT JOIN `x-marketing.jaggaer_pardot_v3.prospects` prospect ON activity.prospect_id = prospect.id
          LEFT JOIN `x-marketing.jaggaer_pardot_v3.campaigns` campaign ON activity.campaign_id = campaign.id -- JOIN
          --   airtable_info airtable
          -- ON
          --   CAST(activity.list_email_id AS STRING) = airtable._pardotid
        WHERE
          activity.type_name = 'Email'
          AND activity.type IN (14) -- AND email NOT LIKE '%@2x.marketing%' 
          -- AND email NOT LIKE '%2X%' 
          -- AND email NOT LIKE '%@jaggaer.com'
...
FROM
  (
    ...
    FROM
      spam_report
  ) AS engagements -- LEFT JOIN campaign_info ON CAST(engagements._campaignID AS STRING) = campaign_info._pardotid
  --LEFT JOIN airtable_info ON engagements._campaignCode = airtable_info._code
  LEFT JOIN prospect_info ON CONCAT(
    engagements._prospectID,
    LOWER(engagements._email)
  ) = CONCAT(
    prospect_info._prospectID,
    LOWER(prospect_info._email)
  );
    ...
    SELECT DISTINCT
      ...
      ROW_NUMBER() OVER (
        PARTITION BY
          ...
          click._list_email_id
        ORDER BY
          click._timestamp DESC
      ) AS rownum
    FROM
      opened_emails AS open
      JOIN clicked_emails AS click ON open._email = click._email
      AND open._campaignID = click._campaignID
      AND open._prospectID = click._prospectID
      AND open._list_email_id = click._list_email_id
  ) scenario
WHERE
  origin._prospectID = scenario._prospectID
  AND origin._email = scenario._email
...
CREATE OR REPLACE TABLE
  `x-marketing.jaggaer.db_lead_campaign_pardot` AS
WITH
  prospects_info AS (
    SELECT
      * EXCEPT (_rownum)
    FROM
      (
        SELECT
          ...
          CASE
            WHEN LOWER(job_title) LIKE LOWER("%Senior Partner Marketing Manager%") THEN "Manager"
            WHEN LOWER(job_title) LIKE LOWER(
              "%Principal Marketing Manager, Strategic Partnerships%"
            ) THEN "Manager"
            WHEN LOWER(job_title) LIKE LOWER("%Head of Channel Success%") THEN "Manager"
            WHEN LOWER(job_title) LIKE LOWER("%Channel Partnerships Manager%") THEN "Manager"
            WHEN LOWER(job_title) LIKE LOWER("%Partner Marketing Program Manager%") THEN "Manager"
            ...
            AND LOWER(job_title) LIKE LOWER("%Senior Partner Marketing Manager%") THEN "Manager"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Principal Marketing Manager%") THEN "Manager"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Strategic Partnerships%") THEN "Manager"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Partner Marketing Program Manager%") THEN "Manager"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Channel Partnerships Manager%") THEN "Manager"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Assistant to%") THEN "Non-Manager"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Senior Counsel%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%General Counsel%") THEN "C-Level"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Founder%") THEN "C-Level"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%C-Level%") THEN "C-Level"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%CDO%") THEN "C-Level"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%CIO%") THEN "C-Level"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%CMO%") THEN "C-Level"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%CFO%") THEN "C-Level"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%CEO%") THEN "C-Level"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Chief%") THEN "C-Level"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%coordinator%") THEN "Non-Manager"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%COO%") THEN "C-Level"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Sr. V.P.%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Sr.VP%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Senior-Vice Pres%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%srvp%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Senior VP%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%SR VP%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Sr Vice Pres%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Sr. VP%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Sr. Vice Pres%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%S.V.P%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Senior Vice Pres%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Exec Vice Pres%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Exec Vp%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Executive VP%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Exec VP%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Executive Vice President%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%EVP%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%E.V.P%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%SVP%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%V.P%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Vice President%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Vice Pres%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%V P%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%VP%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%President%") THEN "C-Level"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Director%") THEN "Director"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%CTO%") THEN "C-Level"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Dir%") THEN "Director"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Dir.%") THEN "Director"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%MDR%") THEN "Non-Manager"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%MD%") THEN "Director"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%GM%") THEN "Director"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Head%") THEN "VP"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Manager%") THEN "Manager"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%escrow%") THEN "Non-Manager"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%cross%") THEN "Non-Manager"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%crosse%") THEN "Non-Manager"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Partner%") THEN "C-Level"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%CRO%") THEN "C-Level"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Chairman%") THEN "C-Level"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Owner%") THEN "C-Level"
            WHEN job_title IS NULL
            AND LOWER(job_title) LIKE LOWER("%Team Lead%") THEN "Manager"
            ELSE job_title
          END AS _seniority,
          prospect.company AS _company,
          prospect.industry AS _industry,
        ...
        FROM
          ...
          LEFT JOIN `x-marketing.jaggaer_pardot_v3.campaigns` campaign ON prospect.campaign_id = campaign.id
        WHERE
