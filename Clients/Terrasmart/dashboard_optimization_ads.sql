TRUNCATE TABLE `x-marketing.terrasmart.dashboard_optimization_ads`;
INSERT INTO `x-marketing.terrasmart.dashboard_optimization_ads` (
  _adid, 
  _adname, 
  _campaignid, 
  _campaignname, 
...
linkedin_ads AS (
    ...
    FROM
      `x-marketing.terrasmart_linkedin_ads_v2.ad_analytics_by_creative`
    WHERE 
  ...
  ads_title AS (
    ...
    campaign_id
  FROM
    `x-marketing.terrasmart_linkedin_ads_v2.creatives`
  ),
  campaigns AS (
    SELECT 
    id AS _campaignID,
    name AS _campaignname
    FROM `x-marketing.terrasmart_linkedin_ads_v2.campaigns`
),
linkedin_airtable AS (
  SELECT
    ...
    _screenshot
  FROM
   `x-marketing.terrasmart_mysql_2.optimization_airtable_ads_linkedin`
  WHERE 
