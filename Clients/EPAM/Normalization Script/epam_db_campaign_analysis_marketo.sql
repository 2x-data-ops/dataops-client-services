...
sent_email AS (
  ...
  QUALIFY ROW_NUMBER() OVER( 
    ...
    ORDER BY activity.activitydate DESC) = 1
),
delivered_email AS ( 
  SELECT
    activity._sdc_sequence,
...
new_delivered_email AS ( 
    ...
    LEFT JOIN soft_hard_email b 
      ON d.campaignID = b.campaignID 
      AND d._leadid = b._leadid
    WHERE b.campaignID IS NULL 
      AND b._leadid IS NULL
),
--delivered based on current smart list
overall_delivered AS ( 
  SELECT n.* 
  FROM new_delivered_email n
  LEFT JOIN `x-marketing.epam_marketo.activities_delete_lead` d 
    ON CAST(d.leadid AS STRING) = _leadid
...
SELECT 
  engagements.* EXCEPT (_description,_email),
  asset_info._utm_source,
  asset_info._utm_medium,
  asset_info._subject, 
  asset_info._campaignname,
  CASE 
    WHEN LENGTH(asset_info._livedate) > 0 
  ...
  CASE 
    WHEN region = 'EMEA' AND asset_info._campaignname LIKE '%Adobe%' THEN 'April Leatherman' 
    ...
    WHEN region = 'MENA' AND asset_info._campaignname LIKE '%Sitecore%' THEN 'TBD' 
    ELSE NULL 
  END AS _partnermarketing,
  asset_info._marketingManager,
  asset_info._industrybusinesstopic,
  asset_info._contenttype
FROM engagements
LEFT JOIN asset_info 
  ON engagements.campaignID = asset_info._pardotid
LEFT JOIN prospect_info 
  ON LOWER(engagements._leadid) = LOWER(prospect_info._leadid)
;
...
FROM (
  SELECT
    CASE 
      WHEN TIMESTAMP_DIFF(click._timestamp, open._timestamp, SECOND) <= 2 THEN click._email 
      ELSE NULL 
    END AS _email, 
    click._utm_campaign 
  FROM `x-marketing.epam.db_campaign_analysis_marketo` AS click
  JOIN `x-marketing.epam.db_campaign_analysis_marketo` AS open 
...
FROM (
  ...
  final_engagement AS (
    ...
    FROM (
      SELECT 
        *, 
        ROW_NUMBER() OVER(PARTITION BY _email, _description ORDER BY _priority DESC) AS _rownum
...
FROM (
  ...
  FROM (
    ...
    FROM `x-marketing.epam.db_campaign_analysis_marketo`
    WHERE _engagement IN ('Delivered', 'Bounced', 'Soft Bounced')
    GROUP BY 1, 2, 3
  )
  ...
  SELECT 
    ...
    CASE 
      WHEN LOWER(title) LIKE '%journalist%' OR LOWER(title) LIKE '%reporter%' OR LOWER(title) LIKE 'student' 
        ...
        OR LOWER(title) LIKE 'machine learning intern' OR LOWER (title) LIKE '%publication%' OR LOWER(title) LIKE 'freelance' 
...
FROM leads
JOIN email_lead 
  ON email_lead._prospectID = CAST(leads.id AS STRING)
;
...
WITH overall_lead AS (
  ...
  WHERE  email  NOT LIKE '%@2x.marketing%' 
    ...
    AND email NOT LIKE 'skylarulry@yahoo.com' 
    AND email NOT LIKE '%test%' 
    AND marketingsuspended IS false
...
cold_lead AS (
  SELECT 
    ...
    'Cold' AS contacttype
  FROM overall_lead o
  LEFT JOIN warm_lead w 
    ON w._prospectID = o._prospectID
...
email_engagement AS (
  ...
  FROM ( 
    ...
    FROM (SELECT * FROM `epam.db_campaign_analysis_marketo`)
    WHERE LOWER(_engagement) NOT IN ('sent','delivered', 'bounced', 'unsubscribed')
  ) a
  WHERE NOT REGEXP_CONTAINS(_domain,'2x.marketing|gmail|yahoo|outlook|hotmail') 
...
dummy_dates AS ( 
  ...
  FROM UNNEST(GENERATE_DATE_ARRAY('2022-01-01', CURRENT_DATE(), INTERVAL 1 DAY)) AS _date 
),
contact_engagement AS (
  SELECT DISTINCT 
    ...
    engagements._date
  FROM dummy_dates
  JOIN (SELECT * FROM email_engagement) engagements 
    USING(_week, _year)
  JOIN tam_contacts 
    USING(_email) 
),
