...
WITH contacts AS (
  SELECT * EXCEPT (rownum) FROM (
    SELECT 
      ...
      CASE   
        WHEN LOWER(marketo.title) LIKE LOWER("%Senior Partner Marketing Manager%") THEN "Manager"
      ...
      marketo.state,
      CASE 
        WHEN marketo.country = 'Unknown value' THEN 'Other'
        ...
        WHEN marketo.country LIKE '%South Korea%' THEN 'Korea, Republic of' 
        ELSE marketo.country 
      END AS country,
      c.region,
      '' AS _persona,
      '' AS _lifecycleStage,
    ...
    LEFT JOIN `x-marketing.epam.db_country` c 
      ON marketo.country = c.country
    LEFT JOIN `x-marketing.epam_mysql.epam_db_db_personsource_lead` l 
      ON l._prospectid = CAST(marketo.id AS STRING)
    WHERE marketo.email  NOT LIKE '%@2x.marketing%' 
      AND marketo.email NOT LIKE '%2X%' 
      AND marketo.email NOT LIKE '%@epam.com' 
      AND marketo.email NOT LIKE 'skylarulry@yahoo.com' 
      AND marketo.email NOT LIKE '%test%' 
      AND marketo.email <> 'sonam.gupta@capgemini.com' 
      AND LOWER(company) <> 'Endava')
  WHERE rownum = 1
),
accounts AS (
...
email_engagement AS (
  SELECT * FROM ( 
    SELECT 
      ...
      _description
    FROM (SELECT * FROM `x-marketing.epam.db_campaign_analysis_marketo`)
    WHERE 
  ...
  WHERE NOT REGEXP_CONTAINS(_domain,'2x.marketing|epam') 
...
web_views AS (
  ...
  find_entry_page AS (
    SELECT * EXCEPT(rownum) FROM (
      SELECT
        *,
        ROW_NUMBER() OVER(PARTITION BY lead_name ORDER BY activity_date) rownum
...
web_engagements AS (
  SELECT
    ...
    _description,
  FROM web_views
  WHERE NOT REGEXP_CONTAINS(LOWER(_first_fullurl), 'unsubscribe')
...
content_engagement AS (
  ...
  FROM epam.web_metrics
  WHERE REGEXP_CONTAINS(LOWER(_page), '/blog/')
),
form_fills AS (
  SELECT 
  ...
  FROM ( 
    SELECT 
      ...
      referrer_url,
      ROW_NUMBER() OVER(PARTITION BY leadid, primary_attribute_value_id ORDER BY activitydate DESC) AS rownum
    FROM `x-marketing.epam_marketo.activities_fill_out_form` fill
    LEFT JOIN `epam_marketo.leads` main 
      ON fill.leadid = main.id
...
first_party_score AS (
  ...
  FROM `epam.account_90days_score`
