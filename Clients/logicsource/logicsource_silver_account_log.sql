...
WITH contact_experience AS (
  SELECT
    *
  FROM `x-marketing.logicsource_mysql.db_contact_experience`
...
account AS (
  SELECT
    ...
    END*/
    associated_company.properties.all_industry.value AS _industry,
    ...
    _interestingtopicsevent,
    _interestingtopicseventclassification,
    contact_experience._accountid,
    _abmstatus,
    _abmlink,
    _abmcreateddate AS _abmcreateddate,
  FROM `x-marketing.logicsource_hubspot.contacts` acc
  JOIN contact_experience
    ON CAST(vid AS STRING) = contact_experience._contactid
  LEFT JOIN `x-marketing.logicsource_mysql.db_account_news` news
    ON contact_experience._accountid = news._accountid
  QUALIFY ROW_NUMBER() OVER (
    PARTITION BY acc.associated_company.company_id, vid, _interestingtopicid
    ORDER BY property_lastmodifieddate.value DESC
  ) = 1
),
contact AS (
  SELECT
...
hubspot_activity AS (
  ...
  UNION ALL
  SELECT
    CAST(associated_company.company_id AS STRING),
    property_hs_analytics_last_url.value,
    "Page View",
    CAST(vid AS STRING),
    property_hs_analytics_last_timestamp.value
  FROM `x-marketing.logicsource_hubspot.contacts`
  UNION ALL
  SELECT
    CAST(associated_company.company_id AS STRING),
  ...
  FROM `x-marketing.logicsource_hubspot.contacts` c,
    ...
    JOIN `x-marketing.logicsource_hubspot.forms` forms ON form.value.form_id = forms.guid
  UNION ALL
  SELECT
    CAST(_company_id AS STRING),
    _contentTitle,
    CONCAT("Email ", _engagement),
    _prospectID,
    _timestamp
...
hubspot_activites AS (
  ...
  JOIN contact_ids
    ON hubspot_activity.contact_id = contact_ids._contactid
...
FROM contact
LEFT JOIN hubspot_activites
  ON contact.contact_id = hubspot_activites.contact_id
LEFT JOIN hubspot_data
  ON contact.contact_id = hubspot_data.contact_id
LEFT JOIN account
