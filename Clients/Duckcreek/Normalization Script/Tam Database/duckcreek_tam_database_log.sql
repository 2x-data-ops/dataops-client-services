...
-- CREATE OR REPLACE TABLE duckcreek.db_icp_database_log;
TRUNCATE TABLE `duckcreek.db_icp_database_log` ;
INSERT INTO `duckcreek.db_icp_database_log`
WITH
  ...
  contacts AS (
    ...
    FROM (
      SELECT 
          ...
          COALESCE(CAST(property_annualrevenue.value AS STRING), associated_company.properties.annualrevenue.value ) AS _revenue,
          ...
          COALESCE(property_salesforceaccountid.value,properties.salesforceaccountid.value) AS _sfdcaccountid,
        ...
        FROM 
          `x-marketing.duckcreek_hubspot.contacts` hs
        WHERE 
