...
TRUNCATE TABLE `x-marketing.duckcreek.db_email_engagements_log`;

INSERT INTO `x-marketing.duckcreek.db_email_engagements_log` (
  _sdc_sequence,
  _campaignID,
  _utmcampaign,
  _subject,
...
WITH prospect_info AS (
  ...
  FROM(
    ...
    FROM 
      `x-marketing.duckcreek_hubspot.contacts` contacts
    WHERE
      property_email.value IS NOT NULL
      AND property_email.value NOT LIKE '%2x.marketing%'
      AND property_email.value NOT LIKE '%duckcreek'
  ...
  WHERE _rownum = 1
  AND _domain NOT IN ('duckcreek.com', 'duckcreek', '2x.marketing')
),
...
airtable_info AS (
  SELECT
    ...
    name,
    CASE 
      WHEN id IN (247413984, 247366742, 247239377, 245978650, 245935666, 245977325, 245936306, 245369808, 245369806, 245369807, 244174255) THEN '2X' END AS _2x_campaign,
    FROM `x-marketing.duckcreek_hubspot.campaigns` campaign
),
--ENGAGEMENT METRIC SEGMENT
email_sent AS (
  SELECT * EXCEPT (_rownum)
  FROM (
    SELECT
...
email_download AS (
  ...
  FROM (
    SELECT
      activity._sdc_sequence,
      CAST(campaign.id AS STRING) AS _campaignID,
      COALESCE(form_title, campaign.name) AS _utmcampaign,
      campaign.subject,
      activity.email AS _email,
      activity.timestamp AS _timestamp,
      'Downloaded' AS _engagement,
    ...
    FROM (
      SELECT
        ...
        CAST(NULL AS STRING) AS devicetype,
        SPLIT(SUBSTR(form.value.page_url, STRPOS(form.value.page_url, '_content=') + 9), '&')[ORDINAL(1)] AS _campaignID,
        REGEXP_REPLACE(REGEXP_REPLACE(SPLIT(SUBSTR(form.value.page_url, STRPOS(form.value.page_url, 'utm_campaign') + 13), '&')[ORDINAL(1)], '%20', ' '), '%3A',':') AS _utmcampaign,
        SPLIT(SUBSTR(form.value.page_url, STRPOS(form.value.page_url, '_source=') + 8), '&')[ORDINAL(1)] AS _utm_source,
        form.value.title AS form_title,
        properties.email.value AS email,
        form.value.timestamp AS timestamp,
...
SELECT
  engagement.*,
  prospect_info.* EXCEPT (_email)
FROM engagements_combined as engagement
LEFT JOIN prospect_info
ON engagement._email = prospect_info._email
JOIN airtable_info
ON engagement._campaignID = CAST(airtable_info.id AS STRING);

--UPDATES--
--Label Bots
UPDATE `x-marketing.duckcreek.db_email_engagements_log` origin
SET origin._isBot = 'Yes'
FROM (
  SELECT
    CASE WHEN TIMESTAMP_DIFF(click._timestamp, open._timestamp, SECOND) <= 10 THEN click._email
    ...
    END AS _email,
    click._utmcampaign
  FROM `x-marketing.duckcreek.db_email_engagements_log` AS click
  JOIN `x-marketing.duckcreek.db_email_engagements_log` AS open
    ON LOWER(click._email) = LOWER(click._email)
  ...
  WHERE click._engagement = 'Clicked'
    ...
    EXCEPT DISTINCT
      ...
      FROM `x-marketing.duckcreek.db_email_engagements_log` AS conversion
      WHERE conversion._engagement = 'Downloaded'
) bot
  ...
  WITH focused_engagement AS(
    SELECT
      ...
      CASE  WHEN _engagement = 'Opened' THEN 1
            ...
            WHEN _engagement = 'Downloaded' THEN 3
            END AS _priority
    FROM `x-marketing.duckcreek.db_email_engagements_log`
    WHERE _engagement IN ('Opened', 'CLicked', 'Downloaded')
    ORDER BY 1, 3, 4 DESC
  ),
  final_engagement AS (
    SELECT * EXCEPT (_priority, _rownum)
    FROM (
      SELECT *, ROW_NUMBER() OVER (PARTITION BY _email, _utmcampaign ORDER BY _priority DESC) AS _rownum
      FROM focused_engagement
      )
    WHERE _rownum = 1
  )
...
WHERE origin._email = final._email
  AND origin._engagement = final._engagement
  AND origin._utmcampaign = final._utmcampaign;
...
WHERE origin._email = scenario._email
  AND origin._utmcampaign = scenario._utmcampaign
  AND origin._engagement IN ('Delivered', 'Bounced');
