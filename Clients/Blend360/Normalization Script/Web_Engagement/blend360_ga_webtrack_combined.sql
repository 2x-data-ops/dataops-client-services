...
INSERT INTO `x-marketing.blend360.ga_webtrack_combined_data` (
  ...
  WITH web_visits_data AS (
    SELECT
      ...
      1 AS _order
    FROM `x-marketing.blend360_google_analytics.Dealfront_Company_Data`
    WHERE ga_pagepath IS NOT NULL
    UNION ALL
    SELECT
      ga_date AS _date,
      ...
      2 AS _order
    FROM `x-marketing.blend360_google_analytics.Demandbase_Company_Data`
    WHERE ga_pagepath IS NOT NULL
    UNION ALL
    SELECT DISTINCT
      ...
      COUNT(_entryPage) OVER(PARTITION BY _domain, DATE(_timestamp)) AS _totalPages,
      COUNT(DISTINCT _entryPage) OVER(PARTITION BY _domain, DATE(_timestamp)) AS _uniquePages,
      CAST(NULL AS FLOAT64),
      'Webtrack' AS _data_source,
      3 AS _order
    FROM `x-marketing.blend360.dashboard_mouseflow_kickfire`
  ),
  remove_duplicate_web_visits AS (
    SELECT
      *
    FROM web_visits_data
    WHERE _company_website IS NOT NULL
    QUALIFY ROW_NUMBER() OVER(PARTITION BY _date, _company_website, _page_url ORDER BY _order) = 1
    UNION ALL
    SELECT
      *
    FROM web_visits_data
    WHERE _company_website IS NULL
  ),
  gsheet_mapping AS (
    SELECT
      ...
      bu AS _bu
    FROM `x-marketing.blend360_google_sheet.Mapping`
  ),
  combined_data AS (
    SELECT
      main.* EXCEPT (_order),
      ...
      CASE
        ...
        ELSE 'Non-Targeted'
      END AS _target_type
    FROM remove_duplicate_web_visits AS main
    LEFT JOIN gsheet_mapping AS side
      ON main._company_website = side._company_website
  )
SELECT
  *
FROM combined_data;