...
SELECT DISTINCT content.* EXCEPT ( _id,_sdc_batched_at, _sdc_received_at,_sdc_sequence, _sdc_table_version),
...
FROM
  (
    SELECT
      DISTINCT *
    FROM
      `x-marketing.blend360_mysql.content_wise_mapping`
    WHERE
      _url != ''
  ) AS content
LEFT JOIN
...
FROM
  `blend360.db_content_engagements_log` content,
