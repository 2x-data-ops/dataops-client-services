...
UPDATE
  `x-marketing.sbi.dashboard_mouseflow_kickfire` sbi_msflow
SET
  sbi_msflow._domain    = kickfire._website,
  sbi_msflow._name       = kickfire._name,
  sbi_msflow._location   = kickfire._location,
  sbi_msflow._industry   = kickfire._category,
  sbi_msflow._revenue    = kickfire._revenue,
  sbi_msflow._employees  = kickfire._employees,
  sbi_msflow._phone      = kickfire._phone
FROM (
  SELECT
    _ipaddr,
    _website,
  ...
  WHERE
    _isisp = 0 ) kickfire
WHERE
  sbi_msflow._ipAddr = kickfire._ipaddr
  AND (LENGTH(sbi_msflow._domain) = 0
    OR sbi_msflow._domain IS NULL);
