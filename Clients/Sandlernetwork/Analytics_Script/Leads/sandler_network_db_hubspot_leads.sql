...
WITH companies AS (
  ...
  FROM `x-marketing.sandler_network_hubspot.companies`
),
...
contacts AS (
  SELECT
    ...
    1 AS dummy_colum
  FROM `x-marketing.sandler_network_hubspot.contacts`
...
combined_data AS (
  ...
  LEFT JOIN companies AS side
    ON main.split_nbc_assignment_all_franchise_hs_portal_ids = side.franchise_child_hs_portal_id
),
...
all_leads_and_mqls AS (
  ...
  SELECT
    ...
    'Lead' AS status
  FROM `x-marketing.sandler_network_hubspot.contacts`
  WHERE property_createdate.value IS NOT NULL
  UNION ALL
  -- Get all MQL dates
  SELECT
    vid AS contact_id,
    ...
    'MQL' AS status
  FROM `x-marketing.sandler_network_hubspot.contacts`
  WHERE property_hs_lifecyclestage_marketingqualifiedlead_date.value IS NOT NULL
),
...
duplicate_rows AS (
  ...
  JOIN all_leads_and_mqls AS side
    USING (contact_id)
)
SELECT
  *
