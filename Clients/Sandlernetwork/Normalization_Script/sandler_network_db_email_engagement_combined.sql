...
network_sent AS (
      ...
      SELECT network_sent_source .* FROM network_sent_source
LEFT JOIN network_hard_bounced_source ON network_sent_source._email = network_hard_bounced_source._email and network_sent_source._campaignID = network_hard_bounced_source._campaignID
...
sandler_sent AS (
     SELECT sandler_sent_source .* FROM sandler_sent_source
LEFT JOIN sandler_hard_bounced_source ON sandler_sent_source._email = sandler_hard_bounced_source._email and sandler_sent_source._campaignID = sandler_hard_bounced_source._campaignID
...
network_delivered AS (
 SELECT network_delivered_source .* FROM network_delivered_source 
LEFT JOIN network_hard_bounced_source ON network_delivered_source._email = network_hard_bounced_source._email and network_delivered_source._campaignID = network_hard_bounced_source._campaignID
WHERE network_hard_bounced_source._email IS NULL
),
...
sandler_delivered AS (
 SELECT sandler_delivered_source .* FROM sandler_delivered_source 
LEFT JOIN sandler_hard_bounced_source ON sandler_delivered_source._email = sandler_hard_bounced_source._email and sandler_delivered_source._campaignID = sandler_hard_bounced_source._campaignID
WHERE sandler_hard_bounced_source._email IS NULL
),
...
sandler_soft_bounced_source AS (
  ...
  sandler_hard_bounced AS (
   SELECT sandler_hard_bounced_source.* 
   FROM sandler_hard_bounced_source
  JOIN sandler_soft_bounced_source ON sandler_hard_bounced_source._email = sandler_soft_bounced_source._email AND sandler_hard_bounced_source._campaignID = sandler_soft_bounced_source._campaignID
),
...
network_hard_bounced AS ( 
   ...
   SELECT network_hard_bounced_source.* 
   FROM network_hard_bounced_source
  JOIN network_soft_bounced_source ON network_hard_bounced_source._email = network_soft_bounced_source._email AND network_hard_bounced_source._campaignID = network_soft_bounced_source._campaignID  
),
...
SELECT *, 'Sandler Network' AS _instance FROM network_hard_bounced
 ...
 sandler_soft_bounced AS (
  SELECT sandler_soft_bounced_source.* FROM sandler_soft_bounced_source
  LEFT JOIN sandler_hard_bounced ON sandler_soft_bounced_source._email = sandler_hard_bounced._email AND sandler_soft_bounced_source._campaignID = sandler_hard_bounced._campaignID
  LEFT JOIN sandler_delivered_source ON sandler_soft_bounced_source._email = sandler_delivered_source._email AND sandler_soft_bounced_source._campaignID = sandler_delivered_source._campaignID
  WHERE sandler_hard_bounced._email IS NULL AND sandler_delivered_source._email IS NULL 
  ),

  network_soft_bounced AS (
  SELECT network_soft_bounced_source.* FROM network_soft_bounced_source
  LEFT JOIN network_hard_bounced_source ON network_soft_bounced_source._email = network_hard_bounced_source._email AND network_soft_bounced_source._campaignID = network_hard_bounced_source._campaignID
  LEFT JOIN network_delivered_source ON network_soft_bounced_source._email = network_delivered_source._email AND network_soft_bounced_source._campaignID = network_delivered_source._campaignID
  WHERE network_hard_bounced_source._email IS NULL AND network_delivered_source._email IS NULL    
  ),
