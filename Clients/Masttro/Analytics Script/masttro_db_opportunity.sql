...
WITH deal_pipeline AS (
  SELECT DISTINCT
    ...
    label AS _pipeline
  FROM
    `x-marketing.masttro_hubspot.deal_pipelines`,
...
opps_created AS (
  ...
  FROM
    `x-marketing.masttro_hubspot.deals` deals
  JOIN deal_pipeline
...
campaign_perf AS (
  ...
  FROM
    `x-marketing.masttro_google_ads.campaign_performance_report` report
...
WITH contacts AS (
  ...
  FROM
    `x-marketing.masttro_hubspot.contacts` contacts
  QUALIFY ROW_NUMBER() OVER (
    PARTITION BY
      property_email.value,
      CONCAT(properties.firstname.value, ' ', properties.lastname.value)
    ORDER BY
      vid DESC
  ) = 1
),
...
aggregate_opp AS (
  ...
  FROM
    `x-marketing.masttro.db_opportunity_aggregate` agg
  LEFT JOIN all_opp
...
blended_opp AS (
  ...
  FROM
    `x-marketing.masttro.google_search_keyword_performance` key_perf
