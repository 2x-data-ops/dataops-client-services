...
-- CREATE OR REPLACE TABLE `simpletire.db_icp_database_log` AS
TRUNCATE TABLE `simpletire.db_icp_database_log` ;
INSERT INTO `simpletire.db_icp_database_log`
WITH
  ...
  contacts AS (
    ...
    FROM (
      SELECT 
        ...
        FROM 
          `x-marketing.simpletire_hubspot.contacts` hs
        )
    WHERE 
      _rownum = 1
  )
