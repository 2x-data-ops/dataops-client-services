-- CREATE OR REPLACE TABLE `x-marketing.thunder.linkedin_ads_performance` AS
TRUNCATE TABLE `x-marketing.thunder.linkedin_ads_performance`;

INSERT INTO `x-marketing.thunder.linkedin_ads_performance` (
...
WITH LI_ads AS (
  ...
  FROM `x-marketing.thunder_linkedin_ads.ad_analytics_by_creative`
...
ads_title AS (
  SELECT
    ...
    campaign_id
  FROM `x-marketing.thunder_linkedin_ads.creatives`
),
campaigns AS (
  SELECT
    ...
    campaign_group_id
  FROM `x-marketing.thunder_linkedin_ads.campaigns`
),
campaign_group AS (
  SELECT
  ...
  FROM `x-marketing.thunder_linkedin_ads.campaign_groups`
),
-- airtable_ads AS (
--     SELECT 
--         _adid,
...
combine_all AS (
  ...
  -- JOIN airtable_ads 
  -- ON LI_ads.li_creative_id = airtable_ads._adid
),
total_ads_per_campaign AS (
  SELECT
    *,
    COUNT(li_creative_id) OVER (PARTITION BY li_run_date, li_campaign_name) AS fm_ads_per_campaign
  FROM combine_all
),
