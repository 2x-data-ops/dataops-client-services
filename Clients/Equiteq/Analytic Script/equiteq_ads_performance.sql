...
WITH unique_rows AS (
	SELECT
		...
		campaign_name,
		DATE(date) AS day,
		--MIS: convert to DATE since timestamps are all showing 00:00:00 UTC
	...
	FROM
		`x-marketing.equiteq_google_ads.campaign_performance_report` -- WHERE campaign_advertising_channel_type = 'SEARCH'
	QUALIFY RANK() OVER (
		PARTITION BY
			date,
			campaign_id
