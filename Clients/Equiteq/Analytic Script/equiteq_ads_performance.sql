begin /* LinkedIn Ad Performance */
  ...
  total_ads_per_campaign AS (
    SELECT *,
    COUNT(li_creative_id) OVER (PARTITION BY li_run_date, li_campaign_name)
...
begin -- Google Search Campaign Performance --
...
begin -- Google Search Ads Variation Performance --
...
begin -- Google Search Keyword Performance --
  ...
  WITH unique_rows AS (
    ...
    QUALIFY RANK() OVER(PARTITION BY date, campaign_id, ad_group_id, ad_group_criterion_keyword.text 
...
begin -- Google Search Query Performance --
  ...
  WITH unique_rows AS (
    ...
    QUALIFY RANK() OVER(PARTITION BY date, campaign_id, ad_group_id, keyword.info.text, search_term_view_search_term 
    ORDER BY _sdc_received_at DESC) = 1
  )
  SELECT
  campaign_id,
...
begin -- Google Display Campaign Performance --
