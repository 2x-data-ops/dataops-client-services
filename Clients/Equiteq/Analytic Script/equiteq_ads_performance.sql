...
INSERT INTO
	`x-marketing.equiteq.linkedin_ads_performance` (
		li_creative_id,
		li_campaign_id,
		li_campaign_name,
		li_campaign_type,
...
WITH
	LI_ads AS (
		SELECT
			...
			--approximate_unique_impressions AS li_reach, -- is in stitch but not in BigQ
		...
		FROM
			`x-marketing.equiteq_linkedin_ads.ad_analytics_by_creative`
	),
	ads_title AS (
		SELECT
		...
		FROM
			`x-marketing.equiteq_linkedin_ads.creatives`
	),
	campaigns AS (
		SELECT
		...
		FROM
			`x-marketing.equiteq_linkedin_ads.campaigns`
	),
	campaign_group AS (
		SELECT
		...
		FROM
			`x-marketing.equiteq_linkedin_ads.campaign_groups`
	),
	combine_all AS (
		SELECT
		...
		FROM
			LI_ads
    RIGHT JOIN ads_title --RIGHT JOIN --to include both launched and yet-to-launch ads
...
INSERT INTO
	`x-marketing.equiteq.google_search_campaign_performance` (
		campaign_id,
		campaign_name,
		day,
		currency,
...
WITH
	unique_rows AS (
		SELECT
			campaign_id,
			campaign_name,
			DATE(date) AS day, --MIS: convert to DATE since timestamps are all showing 00:00:00 UTC
		...
		FROM
			`x-marketing.equiteq_google_ads.campaign_performance_report`
...
INSERT INTO
	`x-marketing.equiteq.google_search_adsvariation_performance` (
		campaign_id,
		campaign_name,
		ad_group_id,
		ad_group_name,
...
WITH
	unique_rows AS (
		SELECT
			...
			customer_time_zone
		FROM
			`x-marketing.equiteq_google_ads.ad_performance_report`
		QUALIFY
			RANK() OVER (
				PARTITION BY
					date,
...
INSERT INTO
	`x-marketing.equiteq.google_search_keyword_performance` (
		campaign_id,
		campaign_name,
		ad_group_id,
		ad_group_name,
...
WITH
	unique_rows AS (
		SELECT
			...
			customer_time_zone,
		FROM
			`x-marketing.equiteq_google_ads.keywords_performance_report`
		QUALIFY
			RANK() OVER (
				PARTITION BY
					date,
...
INSERT INTO
	`x-marketing.equiteq.google_search_query_performance` (
		campaign_id,
		campaign_name,
		ad_group_id,
		ad_group_name,
...
WITH
	unique_rows AS (
		SELECT
			...
			customer_time_zone,
		FROM
			`x-marketing.equiteq_google_ads.search_query_performance_report`
		QUALIFY
			RANK() OVER (
				PARTITION BY
					date,
...
INSERT INTO
	`x-marketing.equiteq.google_display_campaign_performance` (
		campaign_id,
		campaign_name,
		day,
		currency,
