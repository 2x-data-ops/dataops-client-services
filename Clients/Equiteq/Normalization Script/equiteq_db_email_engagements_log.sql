...
WITH prospect_info AS (
  ...
  CASE
    ...
    ELSE
    INITCAP(CAST(properties.lifecyclestage.value AS STRING))
  END
  AS _lifecycleStage
...
Unsubscribed AS (
  SELECT shared_fields.* EXCEPT(_type, id),
  'Unsubscribed' AS _engagement,
  
  FROM `x-marketing.equiteq_hubspot.subscription_changes` , UNNEST(changes) AS status 
  JOIN shared_fields
...
HardBounced AS (
  ...
  JOIN SoftBounced
  ON hb._email = SoftBounced._email AND hb._campaignID = SoftBounced._campaignID
),
...
Sent AS (
  ...
  -- ON sent._email = HardBounced._email AND sent._campaignID = HardBounced._campaignID
  -- LEFT JOIN Dropped
  -- ON sent._email = Dropped._email AND sent._campaignID = Dropped._campaignID
  -- LEFT JOIN Deferred
  -- ON sent._email = Deferred._email AND sent._campaignID = Deferred._campaignID
  -- WHERE HardBounced._email IS NULL AND Dropped._email IS NULL AND Deferred._email IS NULL
),
...
Delivered AS (
  ...
  -- LEFT JOIN HardBounced
  -- ON delivered._email = HardBounced._email AND delivered._campaignID = HardBounced._campaignID
  -- LEFT JOIN Dropped
  -- ON delivered._email = Dropped._email AND delivered._campaignID = Dropped._campaignID
  -- LEFT JOIN Deferred
  -- ON delivered._email = Deferred._email AND delivered._campaignID = Deferred._campaignID
  -- WHERE HardBounced._email IS NULL AND Dropped._email IS NULL AND Deferred._email IS NULL
),
