...
WITH
  ...
  user AS (
    SELECT
      id,
      name AS _owner_name
    FROM `x-marketing.jellyvision_salesforce.User` 
),
  combine AS (
...
WITH 
 ...
 toc AS (
  ...
  contact AS (
    ...
    FROM `x-marketing.jellyvision.db_contacts_accounts_log`
) ,
 opportunity_contact AS (
  ...
  FROM opportunity
  LEFT JOIN contact 
    ON opportunity.accountid = contact._accountid
) 
  SELECT * 
    EXCEPT(_createddate) ,
    CASE 
