...
, buying_stage_data AS (
    ...
    FROM 
        `x-marketing.jellyvision_mysql.jellyvision_db_account_initial_buying_stage`
    WHERE
...
, reached_accounts_data AS (
    ...
    FROM 
        `x-marketing.jellyvision_mysql.jellyvision_db_campaign_reached_accounts` main
...
, sales_intelligence_data AS (
    ...
    JOIN (
        ...
        FROM 
            `x-marketing.jellyvision_mysql.jellyvision_db_sales_intelligence_activities` sales
...
WITH target_account_engagements AS (
    ...
    FROM 
        `jellyvision.db_6sense_engagement_log` 
...
, opps_created AS (
    ...
    FROM 
        `jellyvision_salesforce.Opportunity` opp
    ...
    LEFT JOIN
        `jellyvision_salesforce.User` own
    ON 
        opp.ownerid = own.id 
...
, opps_historical_stage AS (
    ...
    FROM (
        ...
        FROM
            `jellyvision_salesforce.OpportunityFieldHistory` 
        WHERE
            field = 'StageName'
        AND 
            isdeleted = false
    ...
    JOIN (
        SELECT DISTINCT 
            ...
            stagename
        FROM
            `x-marketing.jellyvision_salesforce.OpportunityHistory`
        WHERE 
            isdeleted = false 
...
, combined_data AS (
    ...
    ON (
            opp._domain LIKE CONCAT('%', REGEXP_EXTRACT(act._6sensedomain, r'^[^.]+'), '%')
        AND 
...
CREATE OR REPLACE TABLE `jellyvision.opportunity_summarized` AS
...
FROM 
    `jellyvision.opportunity_influenced_accelerated`;
