...
,LI_ads AS (
 SELECT
  ...
  FROM
    `jellyvision_linkedin_ads.ad_analytics_by_creative`
...
ads_title AS (
  ...
  FROM
    `jellyvision_linkedin_ads.creatives` c
...
campaigns AS (
  ...
  FROM
    `jellyvision_linkedin_ads.campaigns`
...
campaign_group AS (
  ...
  FROM
    `jellyvision_linkedin_ads.campaign_groups`
),
_all AS (
  SELECT
    LI_ads._start_year, 
...
daily_budget_per_ad_per_campaign AS (
  ...
  SELECT daily_budget_per_ad_per_campaign.*
