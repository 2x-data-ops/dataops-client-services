...
CREATE OR REPLACE TABLE `corcentric.marketo_daily_targets` AS
...
WITH target_category AS (
    ...
    leads AS (
        ...
        -- To exclude leads that have become contacts
        AND email NOT IN (
...
campaign_members AS (
    ...
    FROM 
        `corcentric.marketo_lead_movement` 

    -- Exclude those in first week
    WHERE 
        week_rank != 1
    
    -- Get unique person and campaign combo  
    AND 
        year = EXTRACT(YEAR FROM activity_date)
    AND 
        week = EXTRACT(WEEK FROM activity_date)
    
    -- Get people that exist in salesforce, assumed to be deleted in marketo
    AND 
        found_in_salesforce = true
    
    -- Only interested with people involved in campaigns
    AND 
        campaign IS NOT NULL

    -- Get only 2X campaigns 
    AND
        campaign_category = '2X Campaigns'
...
marketo_lead_score AS (
    ...
    SELECT
        ...
        LAG(ROUND(COALESCE(behavior_score__c, 0), 1)) OVER (
        ...
        MAX(extract_date) OVER (

            PARTITION BY 
                CAST(id AS STRING)

        )
        AS latest_extract_date,

        -- Get the latest lead score of a person
        -- For those that dont exist during current campaign
        MAX(ROUND(COALESCE(leadscore, 0), 1)) OVER (
    ...
    FROM 
        `corcentric.marketo_lead_score_snapshot`

    WHERE
        CAST(id AS STRING) IN (
...
combined_data AS (
    ...
    FROM 
        campaign_members AS campaign 
    
    LEFT JOIN 
        marketo_lead_score AS score 

    -- The score data only have marketo id
    -- Set the range to be a month long [CHANGE CONSTANT HERE]
    ON 
        campaign.marketo_id = score.marketo_id
    AND 
        score.extract_date BETWEEN campaign.campaign_engagement_date AND DATE_ADD(campaign.campaign_end_date, INTERVAL 30 DAY)
    
    LEFT JOIN
        unpivot_target_category AS category 
    
    -- The category data only have salesforce id
    -- The range follows the score range
    ON 
        campaign.salesforce_id = category.salesforce_id
    AND 
        score.extract_date = category.category_date 

    WHERE 
        category.category IS NOT NULL 
    OR 
        (score.behaviour_score - score.previous_behaviour_score) > 0
...
get_start_and_end_score AS (
    ...
    LEFT JOIN 
        marketo_lead_score AS score_1

    -- The score data only have marketo id
    -- Do an exact match of the date for score on those days
    ON 
        main.marketo_id = score_1.marketo_id

    AND 
        main.campaign_start_date = score_1.extract_date

    LEFT JOIN 
        marketo_lead_score AS score_2

    -- The score data only have marketo id
    -- Do an exact match of the date for score on those days
    ON 
        main.marketo_id = score_2.marketo_id

    AND 
        main.campaign_end_date = score_2.extract_date
...
get_lead_status AS (
    ...
    -- Use score date as it is present for both score and category
    AND 
        main.score_date = side.extract_date
    
    -- Remove those that are unqualified
    WHERE 
        side.status != 'Unqualified'
    ...
    SELECT DISTINCT
        ...
        *,
        ...
        DENSE_RANK() OVER(
            ...
            ORDER BY 
                DATE_TRUNC(score_date, WEEK(MONDAY))
