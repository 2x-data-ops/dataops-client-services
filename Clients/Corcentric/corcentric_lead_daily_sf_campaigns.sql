...
get_marketo_id_for_salesforce_members AS (
  ...
  FROM salesforce_campaign_members AS main
  JOIN `x-marketing.corcentric_marketo.leads` AS side
    ON main.sf_lead_or_contact_id = side.sfdcleadid
    OR main.sf_lead_or_contact_id = side.sfdccontactid
),
...
get_marketo_scores AS (
  SELECT
    ...
    LAG(ROUND(leadscore, 1)) OVER (PARTITION BY id ORDER BY extract_date) AS previous_lead_score
...
combined_data AS (
  ...
  FROM get_marketo_id_for_salesforce_members AS main
  JOIN get_marketo_scores AS side
    USING (marketo_id)
)
SELECT
  *
