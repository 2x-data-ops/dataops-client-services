...
score_pivot_table AS (
  ...
  GROUP BY 1, 23, 24, 25, 26, 27, 28, 29, 30, 31
),
...
add_marketo_and_sf_info AS (
  ...
  FROM score_pivot_table AS base -- Join with marketo based on ID 
  JOIN marketo_info AS extra_mkt
    ON base.marketoid = extra_mkt.marketoid -- Join with salesforce based on ID 
  LEFT JOIN sf_info AS extra_sf
    ON (
      extra_mkt.marketoid = extra_sf.marketoid
      AND extra_mkt.sfdcleadorcontactid = extra_mkt.sfdcleadorcontactid
    )
),
...
salesforce_lead_emails AS (
  SELECT DISTINCT
    email
  FROM salesforce_leads_and_contacts
  ...
  SELECT
    ...
    CASE
      WHEN (
        ...
        OR side_email.email IS NOT NULL
      ) THEN TRUE
      ELSE FALSE
    END AS found_in_salesforce
  FROM add_marketo_and_sf_info AS main -- For checking those with emails
  LEFT JOIN salesforce_lead_emails AS side_email
    ON main.email = side_email.email -- For checking those without emails
  LEFT JOIN salesforce_lead_names AS side_name
    ON LOWER(main.name) = LOWER(side_name.name)
    AND main.email IS NULL
),
...
salesforce_2x_campaign_members AS (
  SELECT
    ...
    CASE
      ...
      ) THEN campaign.name -- Campaigns with varying names due to waves 
      WHEN campaign.name LIKE '%2023-01-EM-ARD-2X-__-CFO%' THEN '2023-01-EM-ARD-2X-CFO'
      WHEN campaign.name LIKE '%2023-01-EM-ARD-2X-%-Office-of-the-CFO%' THEN '2023-01-EM-ARD-2X-Office-of-the-CFO'
      WHEN campaign.name LIKE '%2023-01-EM-ARD-2X-%-AP-GF%' THEN '2023-01-EM-ARD-2X-AP-GF'
      WHEN campaign.name LIKE '%2023-01-EM-ARD-2X-%-AP-Proc%' THEN '2023-01-EM-ARD-2X-AP-Proc'
      WHEN campaign.name LIKE '%2023-03-EM-Managed-AR-%' THEN '2023-03-EM-Managed-AR'
      WHEN campaign.name LIKE '%2023-06-EM-2X-%-Managed-Services-Office-of-the-CFO%' THEN '2023-06-EM-2X-Managed-Services-Office-of-the-CFO'
      WHEN campaign.name LIKE '%2023-06-NE-EM-2X-%-Managed-Services-Office-of-the-CFO%' THEN '2023-06-NE-EM-2X-Managed-Services-Office-of-the-CFO'
      WHEN campaign.name LIKE '%2023-09-EM-2X-%-MAR-Office-of-the-CFO%' THEN '2023-09-EM-2X-MAR-Office-of-the-CFO'
      WHEN campaign.name LIKE '%2023-10-NE-EM-2X-%-Managed-AR-25-200M%' THEN '2023-10-NE-EM-2X-Managed-AR-25-200M'
      WHEN campaign.name LIKE '%2023-10-NE-EM-2X-%-Managed-AR-200-500M%' THEN '2023-10-NE-EM-2X-Managed-AR-200-500M'
      WHEN campaign.name LIKE '%2023-10-EM-Managed-AR-25-75M%' THEN '2023-10-EM-Managed-AR-25-75M'
      WHEN campaign.name LIKE '%2023-10-EM-Managed-AR-75-200M%' THEN '2023-10-EM-Managed-AR-75-200M'
      WHEN campaign.name LIKE '%2023-11-EM-2X-%-AP+Payments-Office-of-the-CFO%' THEN '2023-11-EM-2X-AP+Payments-Office-of-the-CFO'
      WHEN campaign.name LIKE '%2024-02-EM-2X-%-MAR-Reg-ICP-Lower-Seniority%' THEN '2024-02-EM-2X-MAR-Reg-ICP-Lower-Seniority'
      WHEN campaign.name LIKE '%2024-02-NE-EM-2X-%-Managed-AR-Phase-2%' THEN '2024-02-NE-EM-2X-Managed-AR-Phase-2'
      WHEN campaign.name LIKE '%2024-02-EM-2X-%-MAR-Office-of-the-CFO-Phase-2%' THEN '2024-02-EM-2X-MAR-Office-of-the-CFO-Phase-2'
      WHEN campaign.name LIKE '%2024-01-EM-S2P-NA-250M-5B%' THEN '2024-01-EM-S2P-NA-250M-5B'
      WHEN campaign.name LIKE '%2024-02-NE-EM-2X-%-S2P-NE-150M-5B%' THEN '2024-02-NE-EM-2X-S2P-NE-150M-5B'
      WHEN campaign.name LIKE '%2024-02-EM-AP+Payments-Office-of-the-CFO-Phase-2%' THEN '2024-02-EM-AP+Payments-Office-of-the-CFO-Phase-2'
      WHEN campaign.name LIKE '%2024-05-NE-EM-2X-%-Managed-AR-5M-and-above%' THEN '2024-05-NE-EM-2X-Managed-AR-5M-and-above'
      WHEN campaign.name LIKE '%2024-05-EM-2X-%-S2P-NA-250M-5B-Phase-2%' THEN '2024-05-EM-2X-S2P-NA-250M-5B-Phase-2'
      WHEN campaign.name LIKE '%2024-07-EM-ARD-2X-%-Procurement-Metrics-that-Matter-in-2024%' THEN '2024-07-EM-ARD-2X-Procurement-Metrics-that-Matter-in-2024'
      WHEN campaign.name LIKE '%2024-07-LWB-NE-S2-P2-Automation-to-Humanization%' THEN '2024-07-LWB-NE-S2-P2-Automation-to-Humanization'
      WHEN campaign.name LIKE '%2024-07-TS-NE-S2P-P2-Finance-Operations-Leadership%' THEN '2024-07-TS-NE-S2P-P2-Finance-Operations-Leadership'
      WHEN campaign.name LIKE '%2024-07-NE-EM-2X-%-S2P-P2-NE-150M-5B%' THEN '2024-07-NE-EM-2X-S2P-P2-NE-150M-5B'
      WHEN campaign.name LIKE '%2024-09-NA-NE-EM-2X-%-MAR-Negative-Trending-DSO%' THEN '2024-09-NA-NE-EM-2X-MAR-Negative-Trending-DSO'
      WHEN campaign.name LIKE '%2024-09-NE-EM-2X-%-MAR-LS-25M-500M%' THEN '2024-09-NE-EM-2X-MAR-LS-25M-500M'
    END AS campaign,
    ...
    COALESCE(DATE(campaign.enddate), CURRENT_DATE()) AS campaign_end_date
  ...
  WHERE (
      ...
      OR campaign.name LIKE '%2023-06-EM-2X-%-Managed-Services-Office-of-the-CFO%' -- New 2X's campaigns with many waves
...
combine_campaign_members AS (
  SELECT
    *
  FROM salesforce_2x_campaign_members
  UNION ALL
  SELECT
    *
  FROM salesforce_non_2x_campaign_members
),
campaign_ranks AS (
  SELECT
    *,
    ...
    DENSE_RANK() OVER ( ORDER BY campaign_start_date) AS campaign_rank
...
add_campaign AS (
  ...
  FROM check_marketo_people_in_salesforce AS main
  LEFT JOIN set_campaign_rank_and_date AS side
    ON main.email = side.email
    AND main.sfdcleadorcontactid = side.sf_campaign_member_id
    AND main.year >= side.activity_year
    AND main.week >= side.activity_week
    AND main.year <= side.campaign_year
    AND main.week <= side.campaign_week
),
...
positive_weekly_score_changes AS (
  SELECT
    * EXCEPT (
    ...
    (
      COALESCE(positive_D1_D7_change, 0) + COALESCE(positive_D2_D1_change, 0) +
      COALESCE(positive_D3_D2_change, 0) + COALESCE(positive_D4_D3_change, 0) +
      COALESCE(positive_D5_D4_change, 0) + COALESCE(positive_D6_D5_change, 0) +
      COALESCE(positive_D7_D6_change, 0)
    ) AS positive_week_change,
    -- All positive weekly score change in behaviour scores
    (
      COALESCE(positive_D1_D7_behaviour_change, 0) + COALESCE(positive_D2_D1_behaviour_change, 0) +
      COALESCE(positive_D3_D2_behaviour_change, 0) + COALESCE(positive_D4_D3_behaviour_change, 0) +
      COALESCE(positive_D5_D4_behaviour_change, 0) + COALESCE(positive_D6_D5_behaviour_change, 0) +
    ...
    (
      COALESCE(positive_D1_D7_demographic_change, 0) + COALESCE(positive_D2_D1_demographic_change, 0) +
      COALESCE(positive_D3_D2_demographic_change, 0) + COALESCE(positive_D4_D3_demographic_change, 0) +
      COALESCE(positive_D5_D4_demographic_change, 0) + COALESCE(positive_D6_D5_demographic_change, 0) +
      COALESCE(positive_D7_D6_demographic_change, 0)
    ) AS positive_week_demographic_change
  FROM positive_daily_score_changes
)
SELECT
  *
FROM positive_weekly_score_changes;