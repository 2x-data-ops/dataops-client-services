...
add_marketo_and_sf_info AS (
    ...
    -- Join with marketo based on ID and date info
    JOIN 
        marketo_info AS extra_mkt
    ON (
        base.marketoid = extra_mkt.marketoid
        AND 
        base.week = extra_mkt.week 
        AND 
        base.year = extra_mkt.year
    )
    ...
    LEFT JOIN 
        sf_info AS extra_sf
    ON 
        base.marketoid = extra_sf.marketoid
...
add_campaign AS (
    ...
    JOIN 
        salesforce_campaign_members AS side
    ON 
        ...
        *,
        ...
        CASE 
            ...
            WHEN ( 
                    week_rank != 1 
                AND sf_lead_status != 'Unqualified' 
                AND (max_week_score >= 55 AND max_week_score < 80) 
            ...
            WHEN (
                    week_rank != 1 
                AND sf_lead_status != 'Unqualified' 
                AND (max_week_score >= 20 AND max_week_score < 55) 
