...
INSERT INTO `x-marketing.devo.db_email_engagements_log` (
  ...
  airtable_info AS (
    SELECT
      ...
      ---get the id from hubspot id to join with the engagement data. not airtableid,
      CASE
        WHEN _emailid IS NULL THEN 'Not 2X Campaign'
        ELSE '2X Campaign'
      END AS _2X_campaign,
  ...
  Dropped AS (
    ...
    WHERE activity.type = 'DROPPED'
      AND campaign.name IS NOT NULL --AND activity.recipient NOT LIKE '%devo.%'
  ...
  Sent AS (
    ...
    WHERE activity.type = 'SENT'
      AND campaign.name IS NOT NULL --AND activity.recipient NOT LIKE '%2x.marketing%'
  ...
  HardBounced AS (
    SELECT
      ...
      CAST(activity.duration AS STRING) AS _duration,
      activity.response AS _response,
    FROM `x-marketing.devo_hubspot.subscription_changes` subs,
      UNNEST (changes) AS status
    JOIN `x-marketing.devo_hubspot.email_events` activity
  ...
  email_delivered AS (
    ...
    ---devo exclude hardbounce if they have respond mailbox full from hardbounce. include into delivered instead. 
    LEFT JOIN hardbounced_mailbox AS HardBounced
  ...
  email_hard_bounced AS (
    ...
    JOIN SoftBounced
      ...
      AND HardBounced._campaignID = SoftBounced._campaignID --LEFT JOIN Delivered ON HardBounced._email = Delivered._email AND HardBounced._campaignID = Delivered._campaignID
  ...
  email_unsubscribed AS (
    SELECT
      ...
      CAST(activity.duration AS STRING) AS _duration,
      activity.response AS _response,
    FROM `x-marketing.devo_hubspot.subscription_changes`,
      UNNEST (changes) AS status
    JOIN `x-marketing.devo_hubspot.email_events` activity
      ON status.value.causedbyevent.id = activity.id
    JOIN `x-marketing.devo_hubspot.campaigns` campaign
