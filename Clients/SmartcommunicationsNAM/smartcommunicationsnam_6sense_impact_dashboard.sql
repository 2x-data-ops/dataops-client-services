...
opps_created AS (
    ...
    opps_main AS (
        SELECT DISTINCT
            ...
            opp.opportunity_influence_type__c,
            opp.contactid,
...
SELECT DISTINCT
    ...
    _is_matched_opp,
    _is_influenced_opp,
    opp.opportunity_influence_type__c AS _marketing_influence_type,
    ...
    _is_stagnant_opp,
    _channel
FROM `smartcommnam.opportunity_influenced_accelerated` opp
LEFT JOIN `x-marketing.smartcomm_salesforce.Lead` leads
    ON opp.contactid = leads.convertedcontactid
LEFT JOIN `x-marketing.smartcomm_salesforce.Campaign` campaign
    ON leads.primary_campaign_source__c = campaign.id;
