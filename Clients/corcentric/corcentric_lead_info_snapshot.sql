...
WITH marketo_leads_with_email AS (
  SELECT
    ...
    CASE
      ...
      ELSE sfdcleadid
    END AS sfdcleadorcontactid,
    sfdcaccountid,
    removed_from_marketo__c AS is_deleted,
    ROW_NUMBER() OVER (PARTITION BY email ORDER BY updatedat DESC) rownum
  FROM `x-marketing.corcentric_marketo.leads`
...
marketo_leads_without_email AS (
  SELECT
    ...
    sfdcaccountid,
    removed_from_marketo__c AS is_deleted,
    ROW_NUMBER() OVER (PARTITION BY mktoname ORDER BY updatedat DESC
    ) AS rownum
  FROM `x-marketing.corcentric_marketo.leads`
  WHERE email IS NULL
...
sf_accounts AS (
  ...
  FROM `x-marketing.corcentric_salesforce.Account`
),
unique_marketo_leads_combined AS (
  SELECT
    *
  FROM unique_marketo_leads_with_email
  UNION ALL
  SELECT
    *
  FROM unique_marketo_leads_without_email
),
...
combined_data AS (
  ...
  LEFT JOIN sf_leads
    ON marketo_leads.sfdcleadid = sf_leads.id
  LEFT JOIN sf_contacts
    ON marketo_leads.sfdccontactid = sf_contacts.id
  LEFT JOIN sf_accounts
    ON marketo_leads.sfdcaccountid = sf_accounts.id
),
  ...
  SELECT
    *,
    CASE
      ...
      WHEN country LIKE '%United States%' THEN 'North America' --- North Europe region
      ...
      WHEN country LIKE '%United Kingdom%' THEN 'North Europe' -- South Europe region
      ...
      WHEN country LIKE '%Switzerland%' THEN 'South Europe' -- Empty country
      WHEN country IS NULL THEN NULL -- Non Marketable
      ELSE 'Non-Marketable'
    END AS region
  FROM combined_data
)
