...
-- CREATE OR REPLACE TABLE `x-marketing.bluemantis.opportunity_influenced_accelerated` AS
TRUNCATE TABLE `x-marketing.bluemantis.opportunity_influenced_accelerated`;
INSERT INTO `x-marketing.bluemantis.opportunity_influenced_accelerated` (
  _account_id,
  _account_name,
  _domain,
  _country,
...
WITH account_engagements AS (
  SELECT DISTINCT 
    ...
    END AS _channel
  FROM `x-marketing.bluemantis.db_6sense_engagement_log`
),
...
opps_created AS (
  ...
  FROM `x-marketing.bluemantis_salesforce.Opportunity` opp
  LEFT JOIN `x-marketing.bluemantis_salesforce.Account` act
    ON opp.accountid = act.id 
  LEFT JOIN `x-marketing.bluemantis_salesforce.User` own
...
opp_field_history AS (
  ...
  FROM `x-marketing.bluemantis_salesforce.OpportunityFieldHistory` 
...
opp_history AS (
  SELECT DISTINCT 
    ...
    stagename
  FROM `x-marketing.bluemantis_salesforce.OpportunityHistory`
...
unique_opps_historical_stage AS (
  ...
  QUALIFY ROW_NUMBER() OVER(
    ...
    ORDER BY _historical_stage_change_timestamp DESC) = 1
),
...
opps_history AS (
  SELECT
    ...
    main._amount,
    main._lead_source,
    main._mql_create_date,
    main._mql_first_date,
...
combined_data AS (
  ...
  LEFT JOIN account_engagements AS act
    ON opp._domain = act._6sense_domain
),
  ...
  SELECT 
    *,
    CASE 
      ...
      AND _stage_movement = 'Upward' THEN TRUE
