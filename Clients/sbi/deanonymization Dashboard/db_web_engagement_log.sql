...
UPDATE
 `x-marketing.sbi.db_web_engagements_log`  sbi_msfw
SET
  sbi_msfw._domain    = kickfire._website,
  sbi_msfw._name       = kickfire._name
...
FROM (
  ...
  WHERE
    _isisp = 0 ) kickfire
WHERE
  sbi_msfw._ipAddr = kickfire._ipaddr
  AND (LENGTH(sbi_msfw._domain) = 0
    OR sbi_msfw._domain IS NULL);
