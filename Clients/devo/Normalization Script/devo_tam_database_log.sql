...
-- CREATE OR REPLACE TABLE devo.db_icp_database_log AS
TRUNCATE TABLE `devo.db_icp_database_log` ;
INSERT INTO `devo.db_icp_database_log`(
  _id,
  _email,
  _name,
  _domain,
  ...
  contacts AS (
    ...
    FROM (
      SELECT 
          ...
          properties.devo___lead_score.value AS _leadScore,
          '' AS _persona,
          CASE
          ...
          associated_company.properties.devo___average_marketing_email_clicks.value AS _emailClicks,
          associated_company.properties.devo___average_marketing_email_opens.value AS _emailOpens,
          associated_company.properties.devo___account_scoring.value AS _accountScoring,
          ROW_NUMBER() OVER( PARTITION BY property_email.value ORDER BY vid DESC) AS _rownum,
        FROM 
          `x-marketing.devo_hubspot.contacts` hs
        -- WHERE 
...
SELECT 
  ...
  _hubspotlink,
  _accountScoring
  -- CAST(_emailClicks AS STRING) AS _emailClicks,
  -- CAST(_emailOpens AS STRING) AS _emailOpens
FROM 
  contacts
/* LEFT JOIN
