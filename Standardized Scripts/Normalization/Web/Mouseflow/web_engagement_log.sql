...
WITH mouseflow AS (
    SELECT DISTINCT
      CASE
        WHEN _recordingurl LIKE "%api-%" THEN REGEXP_REPLACE(_recordingurl, r'api-', '')
        WHEN _recordingurl LIKE "%app%" THEN REGEXP_REPLACE(_recordingurl, r'app', 'us')
        ELSE _recordingurl
      ...
      -- _region,
      _country
    FROM `x-marketing.blend360_mysql.mouseflow_pageviews` main
  ),
  ip_company AS (
    SELECT DISTINCT
      _website AS _domain,
    ...
    FROM `webtrack_ipcompany.webtrack_ipcompany_6sense`
    WHERE _isisp = 0
  ),
  mapped AS (
    SELECT
      * EXCEPT (_country, _domain, _name),
      ip_company._name AS _name,
...
SELECT DISTINCT
  ...
  COALESCE(
    _utmsource,
    REGEXP_EXTRACT(_fullurl, r'[?&]utm_source=([^&]+)')
  ) AS _utmsource,
  COALESCE(
    _utmcampaign,
    REGEXP_EXTRACT(_fullurl, r'[?&]utm_campaign=([^&]+)')
  ) AS _utmcampaign,
  COALESCE(
    _utmmedium,
    REGEXP_EXTRACT(_fullurl, r'[?&]utm_medium=([^&]+)')
  ) AS _utmmedium,
  COALESCE(
    _utmcontent,
    REGEXP_EXTRACT(_fullurl, r'[?&]utm_content=([^&]+)')
  ) AS _utmcontent,
  _utmterm,
  _ipaddr,
  ...
  _region,
  CASE
    WHEN _country = 'us' THEN 'United States'
    WHEN _country = 'US' THEN 'United States'
    ELSE _country
  END AS _country,
 ...
 */
UPDATE `x-marketing.blend360.db_web_engagements_log` origin
  ...
  (
    SELECT DISTINCT
      ...
      CASE
        ...
        WHEN _fullurl LIKE '%blend360-step-one%' THEN 'blend360 Step One'
        WHEN _fullurl LIKE '%state-local%' THEN 'State Local'
        WHEN _fullurl LIKE '%wbn-seclore-blend360%' THEN 'WBN Seclore blend360'
 ...
 */
UPDATE `x-marketing.blend360.db_web_engagements_log`
